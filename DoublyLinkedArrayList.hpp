...
public:
    ...
    explicit DoublyLinkedArrayList(size_t capacity = 10);
...
: nodes((Node *) std::calloc(capacity + 1, sizeof(*nodes))), headPos(0), tailPos(0), sorted(false),
  capacity(capacity)
{
    createFreePosList(1);
...
template<typename T>
size_t DoublyLinkedArrayList<T>::insertBeforePhysicalPos(size_t physicalPos, T val)
{
    VALIDATE_LIST;
...
template<typename T>
size_t DoublyLinkedArrayList<T>::insertBeforeTail(T val)
{
    VALIDATE_LIST;

    size_t freeInsertPos = findFreePos();
    if (freeInsertPos == 0) {
        VALIDATE_LIST;
        return 0;
    }
    ...
    if (tailPos != 0) {
        nodes[tailPos].prev = freeInsertPos;
    } else {
        headPos = freeInsertPos;
    }

    nodes[freeInsertPos].next = tailPos;
    nodes[freeInsertPos].prev = 0;
    nodes[freeInsertPos].data = val;
    tailPos = freeInsertPos;
    sorted = false;
    ++size;

    VALIDATE_LIST;
    return freeInsertPos;
}

template<typename T>
size_t DoublyLinkedArrayList<T>::insertAfterHead(T val)
{
    VALIDATE_LIST;
    ...
    if (headPos != 0) {
        nodes[headPos].next = freeInsertPos;
    } else {
        tailPos = freeInsertPos;
    }

    nodes[freeInsertPos].next = 0;
    nodes[freeInsertPos].prev = headPos;
    nodes[freeInsertPos].data = val;
    headPos = freeInsertPos;
    ++size;
    ...
    for (size_t i = 0; i <= capacity; ++i) {
        ...
        if (i == 0) {
            std::fprintf(dumpFile,
                         ...
                         i, nodes[i].prev, nodes[i].data, nodes[i].next);
            continue;
        }
...
bool DoublyLinkedArrayList<T>::isValid() const
{
    ...
    for (size_t i = 0; i < capacity - size; freePos = nodes[freePos].next, ++i) {
        if ((nodes[freePos].prev != 0) || (nodes[freePos].data != -1)) return false;
    }
